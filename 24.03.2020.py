...
class Pose(Node):
    ...
    def Show(self):
        pose, = plt.plot(self.x, self.y, marker = 'D', color ='c')
        return pose,
...
class SearchSpaceD:
    def __init__(self, d):
        self.d = d
        self.d_dot = 0.1 #I assumed the acceleration for the robot as 0.05 rad/s**2
        self.t = 2       #I assumed the time for generating subtrajectories is 1s
        self.search_space = []
                    ...
                    for v_f_count in range(self.num_v):
                        for p_count in range(self.num_p):
                            ...
                            if min_dist <= self.min_obs_dist: #if there is a collision,
                                ...
                                self.sub_traj_obj.list1[d_r_count][d_f_count][v_r_count][v_f_count][p_count].cost = - 0.5 * min_dist
...
class FullTrajectory:
    ...
    def ShowProfile(self):
        ...
        for i in range(len(self.chosen_d_r_list)):
            ...
            c_v_r = self.chosen_v_r_list[i]
            c_v_f = self.chosen_v_f_list[i]
            self.overall_vel[i] = self.sub_traj_list[i].sub_traj_obj.list1[c_d_r][c_d_f][c_v_r][c_v_f][self.chosen_reced_point].v
            self.v_r_list[i] = self.sub_traj_list[i].sub_traj_obj.list1[c_d_r][c_d_f][c_v_r][c_v_f][self.chosen_reced_point].v_r
            self.v_f_list[i] = self.sub_traj_list[i].sub_traj_obj.list1[c_d_r][c_d_f][c_v_r][c_v_f][self.chosen_reced_point].v_f
...
test_pose = Pose(8, 8, 3 * pi / 2)


#Defining goals
goal_pose_list = []
goal_pose1 = Pose(5, 4, pi)
goal_pose_list.append(goal_pose1)
goal_pose2 = Pose(2, 8, pi/2)
...
test_obs1 = Obstacle(node1, node2, 2)
test_obs2 = Obstacle(node3, node4, 2)
test_obs3 = Obstacle(node5, node6, 2)
test_obs4 = Obstacle(node7, node8, 2)
test_obs5 = Obstacle(node9, node10, 2)
...
start, = plt.plot(test_pose.x, test_pose.y, marker = 'X', color = 'black', ms = 10)
start, = start,
obs = test_obs1.Show()
test_obs2.Show()
test_obs3.Show()
